...
load("bbbclub/correlation_heat_map.RData")  # from R utility programs

# read data from comma-delimited text file... create data frame object
sydney <- read.csv("bbbclub/sydney.csv")
names(sydney) <- 
...
# Pot it to a pdf: models and lowess smooth fits for variable pairs
pdf(file = "bbbclub/fig_predicting_choice_scatter_plot_matrix.pdf", 
...
# correlation heat map for the explanatory variables to a pdf file
pdf(file = "bbbclub/fig_predicting_choice_correlation_heat_map.pdf",
    width = 8.5, height = 8.5)
sydney_cormat <- 
    cor(sydney[, c("Car_Time", "Car_Cost", "Train_Time", "Train_Cost")])
correlation_heat_map(sydney_cormat)
...
#plot predicting choice density
...
#plot predicting choice density to a pdf file

pdf(file = "bbbclub/fig_predicting_choice_density_evaluation.pdf", 
    width = 8.5, height = 8.5)
plotting_object <- densityplot( ~ Predict_Prob_TRAIN | Choice, 
               data = sydney, 
               layout = c(1,2), aspect=1, col = "darkblue", 
...
pdf(file = "bbbclub/fig_predicting_choice_ticket_price_solution.pdf", 
    width = 8.5, height = 8.5) 
